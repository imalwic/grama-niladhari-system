  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#003366] dark:text-blue-400">Household Reports</h1>
          <p className="text-muted-foreground">Request and view comprehensive household reports from GN Officers</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger render={
            <Button className="bg-[#003366] hover:bg-[#002244] dark:bg-blue-600">
              <Plus className="mr-2 h-4 w-4" /> Request Report
            </Button>
          } />
          <DialogContent>
            <form onSubmit={handleRequest}>
              <DialogHeader>
                <DialogTitle>Request Wasama Report</DialogTitle>
                <DialogDescription>
                  Select a Wasama to request a full family and household report. The GN officer will need to approve and generate this report.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label className="text-right">Wasama</Label>
                  <Select value={selectedWasama} onValueChange={(val) => setSelectedWasama(val || "")} required>
                    <SelectTrigger className="col-span-3">
                      <SelectValue placeholder="Select Wasama" />
                    </SelectTrigger>
                    <SelectContent>
                      {wasamas.map(w => (
                        <SelectItem key={w.id} value={w.id}>{w.name || w.code || "Unnamed Wasama"}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              ...
              <TableRow key={report.id}>
                ...
                <TableCell className="text-right">
                  ...
                  {report.status === 'PENDING' && (
                    <span className="text-sm text-muted-foreground italic">Waiting for GN</span>
                  )}
            ...
            {reports.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center text-muted-foreground">
                  {loading ? "Loading..." : "No reports requested yet."}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
